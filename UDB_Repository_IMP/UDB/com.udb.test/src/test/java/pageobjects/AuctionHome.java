...
public class AuctionHome {
	...
	public AuctionHome(WebDriver driver) {
		this.driver = driver;
	}

	public String getAuctionHPageText() {
		return commonObj.getText("textOnAuctionHmPg");
	}

	public boolean verifyDiffColsOnAuctionHmPg() {
		WebDriverWait wait = new WebDriverWait(driver, 7);
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By
				.id("example_processing")));
		List<WebElement> auctionHmPgCols = commonObj
				.getWebElements("auctionHomePgCols");
		String[] columns = new String[] { "Action", "Auction ID", "Target",
				"Seller", "Industry", "Region", "Date", "Status",
				"Latest Update", "Last updated user", "Last updated date",
				"Published", "Updates Published" };
		int i = 0;
		for (WebElement currentColumn : auctionHmPgCols) {
	...
	public int getNumberOfRecordsDisplayedOnAuctionHome() {
		List<WebElement> recordsOnAuctionHome = commonObj
				.getWebElements("numberOfRecordsDispOnAuctionHome");
		return recordsOnAuctionHome.size();
	}

	public boolean adjustNumberOfRecordsToBeDispOnAunctionHm() {
		...
		for (int i = 0; i < showEntriesValues.length; i++) {
			...
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By
					.id("example_processing")));
			List<WebElement> recordsOnAuctionHp = commonObj
					.getWebElements("numberOfRecordsDispOnAuctionHome");
			int temp = Integer.parseInt(showEntriesValues[i]);
			int recordCount = recordsOnAuctionHp.size();
			if (recordCount != temp) {
	...
	public boolean pageNavigation(String recordsToDisplay) {
		...
		/*commonObj.click("clickOnAuctionIdCol");
		wait.until(ExpectedConditions.invisibilityOfElementLocated(By
				.id("example_processing")));*/
		Select select = new Select(
				driver.findElement(By.name("example_length")));
		...
		if (recordCount == temp) {
			String mergerID = commonObj.getText("auctionID");
			for (int j = 0; j < 42; j++) {
				commonObj.click("nextNav");
				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
						.id("example_processing")));
				if (commonObj.getText("auctionID").equals(mergerID)) {
					pageNavigationWorks = false;
					break;
				} else
					mergerID = commonObj.getText("auctionID");
			}
		}
	...
	public boolean diffSearchOnAuctionHome(String searchType) {
		...
		case "byAuctionId":
			String auctionId = commonObj.getText("selectAuctionId");
			commonObj.type("searchByAuctionID", auctionId);
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By
					.id("example_processing")));
			List<WebElement> recordsByAuctionId = commonObj
					.getWebElements("numberOfRecordsDispOnAuctionHome");
			int recordCount = recordsByAuctionId.size();
			if (!(recordCount == 1 && commonObj.getText("getAuctionId").equals(
					auctionId)))
				searchWorks = false;
			break;
		...
		case "byFrom-ToDate":
			try {
				driver.navigate().refresh();
				commonObj.click("searchAuctionFromDate");
				commonObj.click("datePreviousButton");
				commonObj.click("selectDate");
				commonObj.click("searchAuctionToDate");
				commonObj.click("selectDate");
				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
						.id("example_processing")));
		...
		case "byStatus":
			driver.navigate().refresh();
			String[] status = { "M&A Opportunity", "New on the block",
					"Off the block", "Still on the block" };
			for (int i = 0; i < status.length; i++) {
				commonObj.selDropdownElementUsingVisibleText(
						"searchByAuctionStat", status[i]);
				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
						.id("example_processing")));
				List<WebElement> recordsByStatus = commonObj
						.getWebElements("numberOfRecordsDispOnAuctionHome");
				for (WebElement record : recordsByStatus) {
					String auctionStatus = record.findElement(
							By.xpath("./td[8]")).getText();
					if (!(auctionStatus.equals(status[i]))) {
						searchWorks = false;
						break;
					}
				}
		...
		case "byLastUpdatedUser":
			driver.navigate().refresh();
			String updatedUser1 = commonObj.getText("selectAuctionUpdatedUser");
			commonObj.type("searchByAuctionUser", updatedUser1);
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By
					.id("example_processing")));
			updatedUser1 = updatedUser1.toLowerCase();
			List<WebElement> recordsByUpdatedUser = commonObj
					.getWebElements("numberOfRecordsDispOnAuctionHome");
			for (WebElement record : recordsByUpdatedUser) {
				String updatedUser2 = record.findElement(By.xpath("./td[10]"))
						.getText();
				updatedUser2 = updatedUser2.toLowerCase();
				if (!updatedUser2.contains(updatedUser1)) {
					searchWorks = false;
					break;
				}
			}
			while (commonObj.isClickable("nextNav")) {
				...
				recordsByUpdatedUser = commonObj
						.getWebElements("numberOfRecordsDispOnAuctionHome");
				for (WebElement record : recordsByUpdatedUser) {
					String updatedUser2 = record.findElement(
		...
		case "byLastUpdatedDate":
			try {
				driver.navigate().refresh();
				commonObj.click("auctionLastUpdtFrmDt");
				commonObj.click("datePreviousButton");
				commonObj.click("datePreviousButton");
				commonObj.click("datePreviousButton");
				commonObj.click("selectDate");
				commonObj.click("auctionLastUpdtToDt");
				commonObj.click("selectDate");
				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
						.id("example_processing")));
				String fromDate = driver.findElement(
						By.id("example_range_from_10")).getAttribute("value");
				String toDate = driver
						.findElement(By.id("example_range_to_10"))
						.getAttribute("value");
				DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
				Date frmDate = dateFormat.parse(fromDate);
				Date tDate = dateFormat.parse(toDate);
		...
		case "byPublished-Unpublished":
			...
			for (int i = 0; i < 3; i++) {
				if (pubValue[i].equals("")) {
					commonObj.selectDropdwonElement("auctionPubDropdown",
							pubValue[i]);
					wait.until(ExpectedConditions
							.invisibilityOfElementLocated(By
									.id("example_processing")));
					...
					for (WebElement record : publishedRecords) {
						if (!record.findElement(By.xpath("./td[12]")).getText()
								.contains(pubValue[i])) {
							searchWorks = false;
							break;
						}
					...
					for (int j = 0; j < 5 && commonObj.isClickable("nextNav"); j++) {
						...
						publishedRecords = commonObj
								.getWebElements("numberOfRecordsDispOnAuctionHome");
						for (WebElement record : publishedRecords) {
							if (!record.findElement(By.xpath("./td[12]"))
		...
		case "byUpdatesPublished":
			...
			for (int i = 0; i < 4; i++) {
				if (updatesPubValue[i].equals("")) {
					commonObj.selectDropdwonElement(
							"auctionUpdatesPubDropdown", updatesPubValue[i]);
					wait.until(ExpectedConditions
							.invisibilityOfElementLocated(By
									.id("example_processing")));
	...
	public boolean filterByFunctionality(String byFilter) {
		...
		case "filterOnAuctionId":
			...
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By
					.id("example_processing")));
			for (int j = 0; j < 5; j++) {
				String auctionID = commonObj.getText("firstAuctionID");
				int auctionId1 = Integer.parseInt(auctionID);
				List<WebElement> filteredRecords = commonObj
						.getWebElements("numberOfRecordsDispOnAuctionHome");
				for (WebElement record : filteredRecords) {
					String currentAuctionId = record.findElement(
							By.xpath("./td[2]")).getText();
					int auctionId2 = Integer.parseInt(currentAuctionId);
					if (auctionId1 < auctionId2) {
						filterWorks = false;
						break;
					} else {
						auctionId1 = auctionId2;
					}
				}
				commonObj.click("nextNav");
				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
		...
		case "filterOnAnnouncementDate":
			commonObj.click("auctionDtCol");
			wait.until(ExpectedConditions.invisibilityOfElementLocated(By
					.id("example_processing")));
			try {
				for (int j = 0; j < 74; j++) {
					String auctionDate1 = commonObj.getText("firstAuctionDt");
					List<WebElement> filteredRecords = commonObj
							.getWebElements("numberOfRecordsDispOnAuctionHome");
					DateFormat dateFormat = new SimpleDateFormat("MM/dd/yyyy");
					Date date1 = dateFormat.parse(auctionDate1);
					for (WebElement record : filteredRecords) {
						String auctionDate2 = record.findElement(
								By.xpath("./td[7]")).getText();
						Date date2 = dateFormat.parse(auctionDate2);
		...
		case "filterOnStatus":
			...
			for (int j = 0; j < 10; j++) {
				...
				List<WebElement> filteredRecords = commonObj
						.getWebElements("numberOfRecordsDispOnAuctionHome");
				for (WebElement record : filteredRecords) {
					String status2 = record.findElement(By.xpath("./td[8]"))
							.getText();
		...
		case "filterOnLastUpdatedUser":
			...
			for (int j = 0; j < 25; j++) {
				...
				List<WebElement> filteredRecords = commonObj
						.getWebElements("numberOfRecordsDispOnAuctionHome");
				for (WebElement record : filteredRecords) {
					String userName2 = record.findElement(By.xpath("./td[10]"))
							.getText();
